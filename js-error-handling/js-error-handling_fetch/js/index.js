...
async function fetchUserData(url) {
  try {
    ...
    console.log("Content-type issue:", contentType);

    if (response.status === 404) {
      console.log("Network error");
      errorElement.textContent = "Error: User cannot be found (404).";
...
endpoints.forEach((endpoint) => {
  ...
  button.addEventListener("click", async () => {
    ...
    if (result.error) {
      errorElement.textContent = "Unexpected response type: text/html.";
      userElement.innerHTML = "No user data available.";
    } else {
